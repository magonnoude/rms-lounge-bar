...
// Form handling - CORRECTION APPLIQUÉE ICI
const formManager = {
  init: function() {
  ...
  handleFormSubmit: async function(e, formType) {
    ...
    try {
      ...
      if (error.message.includes('Failed to fetch') || error.message.includes('CORS') || error.message.includes('NetworkError')) {
        utils.showNotification('Erreur de connexion. Veuillez vérifier votre connexion internet ou réessayer plus tard.', false);
      } else {
  ...
  prepareFormData: function(form, formType) {
    ...
    // CORRECTION: Utilisation de querySelector avec les bons IDs
    if (formType === 'reservation') {
      ...
      // Vérification que les IDs correspondent à votre HTML
      formData.name = form.querySelector('#contactName') ? form.querySelector('#contactName').value : '';
      formData.email = form.querySelector('#contactEmail') ? form.querySelector('#contactEmail').value : '';
      formData.subject = form.querySelector('#contactSubject') ? form.querySelector('#contactSubject').value : '';
      formData.message = form.querySelector('#contactMessage') ? form.querySelector('#contactMessage').value : '';
      
      console.log('Données formulaire contact:', formData);
    }
