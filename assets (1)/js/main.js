// =============================================================================
// CONFIGURATION PRINCIPALE - Chargée depuis config.js externe
// =============================================================================

// Configuration AWS (chargée depuis le fichier config.js externe)
const API_CONFIG = window.CONFIG?.API_CONFIG || {
  URL: '',
  KEY: ''
};
...
function checkConfig() {
  if (!API_CONFIG.URL || !API_CONFIG.KEY) {
    console.error('Configuration API manquante. Vérifiez le fichier config.js');
    return false;
  }
  return true;
}
...
const formValidator = {
  validateReservation: function(formData) {
    ...
    // Vérifier reCAPTCHA seulement si la clé est configurée
    if (RECAPTCHA_SITE_KEY && !formData.recaptchaToken) {
...
const formManager = {
  ...
  initRecaptcha: function() {
    return new Promise((resolve, reject) => {
      ...
      if (!RECAPTCHA_SITE_KEY) {
        console.warn('Clé reCAPTCHA non configurée');
        resolve();
        return;
      }
  ...
  executeRecaptcha: async function(action) {
    try {
      ...
      if (!RECAPTCHA_SITE_KEY) {
        console.warn('reCAPTCHA non configuré, bypass');
        return 'bypass-token-no-recaptcha';
      }
      ...
      console.log('Token reCAPTCHA généré:', token.substring(0, 20) + '...');
      return token;
    } catch (error) {
  ...
  handleFormSubmit: async function(e, formType) {
    ...
    if (!checkConfig()) {
      utils.showNotification('Erreur de configuration. Veuillez réessayer plus tard.', false);
      return;
    }
    ...
    try {
      // MODIFICATION: Exécuter reCAPTCHA et gérer les erreurs
      try {
        formData.recaptchaToken = await this.executeRecaptcha(formType);
      } catch (recaptchaError) {
        // Si reCAPTCHA échoue mais n'est pas configuré, continuer
        if (!RECAPTCHA_SITE_KEY) {
          formData.recaptchaToken = 'bypass-token-no-recaptcha-config';
        } else {
  ...
  handleNewsletterSubmit: async function(e) {
    ...
    try {
      ...
      // Enregistrer l'email localement
